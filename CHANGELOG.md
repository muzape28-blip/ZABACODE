...
- Patch previews now use a bounded line diff with proper insertion, deletion, and replacement ranges. Large files receive an explicit simplified preview rather than a misleading diff.
- Applying an Oracle patch is now a single Ace edit transaction and exposes a guarded **Undo Oracle Fix** action; a revert is refused if the user has changed the document afterwards.
